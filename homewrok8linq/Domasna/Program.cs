...
namespace HomeWork8
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            FansArray.Add(Maria);
            FansArray.Add(Jane);
            Jerry.AddToFavorites(Songs.Where(song => song.Title.StartsWith("B")).ToList());
            Maria.AddToFavorites(Songs.Where(song => song.Length >= 360).ToList());
            Jane.AddToFavorites(Songs.Where(song => song.MusicGenre == Genre.Rock).ToList());
            Stefan.AddToFavorites(Songs.Where(song => song.Length <= 180 && song.MusicGenre == Genre.Hip_Hop).ToList());
            List<Person> lessThan4Songs = FansArray.Where(person => person.FavoriteSongs.Count <= 4).ToList();
            FansArray.ForEach(fan => { Console.WriteLine(fan.FirstName); fan.GetFavSongs(); Console.WriteLine("---------------------------------------"); });
            lessThan4Songs.ForEach(person => { Console.WriteLine(person.FirstName); });
        }
    }
}
